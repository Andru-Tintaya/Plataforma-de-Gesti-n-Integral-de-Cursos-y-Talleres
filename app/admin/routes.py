...
def dashboard():
    ...
    pagos_pendientes = Inscripcion.query.filter_by(pago_verificado=False).count()
...
# ============ VALIDAR ESTUDIANTES POR ASISTENCIAS ============
@admin_bp.route('/validar-estudiantes')
@login_required
def validar_estudiantes():
    ...
    for curso in cursos:
        ...
        for ins in inscripciones:
            if ins.usuario.rol.nombre == 'estudiante':
                total_sesiones = Sesion.query.filter_by(curso_id=curso.id).count()
    ...
    return render_template('admin/validar_estudiantes.html', estudiantes=estudiantes_data)
# ============ REPORTES ============
